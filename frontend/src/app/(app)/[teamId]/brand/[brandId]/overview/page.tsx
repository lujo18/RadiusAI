...
export default function BrandOverviewPage({ params }: { params: Promise<{ brandId: string; teamId: string }> }) {
  const router = useRouter();
  const { brandId, teamId } = React.use(params);
  const user = useAuthStore((state) => state.user);
  const { data: brands, isLoading, error } = useBrands();
  
  // Redirect to /${teamId}/overview if:
  ...
  }, [brands, isLoading, brandId, router, teamId]);
