...
export default function PricingPage() {
  ...
  return (
    <div className="min-h-screen bg-dark-600">
      ...
      <section className="pb-20 px-6">
        <div className="max-w-7xl mx-auto grid md:grid-cols-3 gap-8">
          ...
          <div className="glass-card p-8 relative">
            <div className="mb-6">
              <h3 className="text-2xl font-bold text-white mb-2">{pricingContent.plans.starter.name}</h3>
              <p className="text-gray-400 text-sm">{pricingContent.plans.starter.description}</p>
            </div>
            ...
            <ul className="space-y-3">
              {pricingContent.plans.starter.features.map((feature, i) => (
                <li key={i} className="flex items-start text-gray-300">
                  <FiCheck className="text-primary-500 mr-3 mt-1 flex-shrink-0" />
          ...
          {/* GROWTH PLAN (POPULAR) */}
          <div className="glass-card p-8 relative border-2 border-primary-500 transform md:scale-105">
            ...
            <div className="mb-6">
              <h3 className="text-2xl font-bold text-white mb-2">{pricingContent.plans.growth.name}</h3>
              <p className="text-gray-400 text-sm">{pricingContent.plans.growth.description}</p>
            </div>
          ...
          <div className="glass-card p-8 relative">
            ...
            <div className="mb-6">
              <h3 className="text-2xl font-bold text-white mb-2">{pricingContent.plans.unlimited.name}</h3>
              <p className="text-gray-400 text-sm">{pricingContent.plans.unlimited.description}</p>
            </div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-wrap items-center justify-center gap-8">
            {pricingContent.socialProof.companies.map((company, i) => (
              <div
                key={i}
                className="w-16 h-16 rounded-full bg-gray-800 border border-gray-700 flex items-center justify-center"
              >
              ...
              <thead>
                <tr className="border-b border-gray-800">
                  ...
                  <th className="text-center py-4 px-4">
                    ...
                    <div className="text-gray-400 text-sm">$19/mo</div>
                  </th>
                  <th className="text-center py-4 px-4 bg-primary-500/10 rounded-t-lg">
                    <div className="text-white font-bold mb-1">Growth</div>
                    <div className="text-primary-400 text-sm">${growthPrice?.amount?.toFixed(0) || '29'}/mo</div>
                  </th>
                  <th className="text-center py-4 px-4">
                    <div className="text-white font-bold mb-1">Unlimited</div>
                    <div className="text-gray-400 text-sm">${unlimitedPrice?.amount?.toFixed(0) || '99'}/mo</div>
                  </th>
                </tr>
              </thead>
              <tbody>
                {pricingContent.comparison.categories.map((category, catIndex) => (
                  <>
                    <tr key={`cat-${catIndex}`} className="border-b border-gray-800">
                      <td colSpan={4} className="py-4 px-4 text-white font-bold text-lg">
                        {category.name}
                      </td>
                    </tr>
                      ...
                      <tr key={`feat-${catIndex}-${featIndex}`} className="border-b border-gray-800/50">
                        ...
                        <td className="py-3 px-4 text-center">
                          {typeof feature.starter === 'boolean' ? (
                            feature.starter ? (
                              <FiCheck className="text-primary-500 mx-auto" />
                            ) : (
                        ...
                        <td className="py-3 px-4 text-center bg-primary-500/5">
                          {typeof feature.starter === 'boolean' ? (
                            feature.starter ? (
                              <FiCheck className="text-primary-500 mx-auto" />
                            ) : (
      ...
      {/* FOOTER */}
      <footer className="bg-dark-600 border-t border-gray-800 py-12 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="grid md:grid-cols-4 gap-8 mb-8">
            <div>
              ...
              <p className="text-gray-400 text-sm">AI-powered carousel automation for modern creators</p>
            </div>

            <div>
              <h3 className="text-white font-semibold mb-4">Product</h3>
              <ul className="space-y-2">
                <li><Link href="/#benefits" className="text-gray-400 hover:text-white text-sm transition">Features</Link></li>
                <li><Link href="/pricing" className="text-gray-400 hover:text-white text-sm transition">Pricing</Link></li>
            ...
            <div>
              ...
              <ul className="space-y-2">
                <li><Link href="/privacy" className="text-gray-400 hover:text-white text-sm transition">Privacy</Link></li>
                <li><Link href="/terms" className="text-gray-400 hover:text-white text-sm transition">Terms</Link></li>
              </ul>
            </div>
          </div>
