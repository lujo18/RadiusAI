...
export default function StockPacksDialog({
  ...
  const { data: packImagesRaw = [] } = useStockPackImages(viewingPack?.bucket_directory || null);
  ...
  const handleUsePack = () => {
    if (isSelectionMode && (selectedPackId || viewingPackId)) {
      ...
      if (pack && pack.bucket_directory) {
        ...
        console.error('Pack not found for ID:', packId, 'Available:', activePacks.map(p => ({ id: p.id, dir: p.bucket_directory })));
      }
    }
  };
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-muted-foreground mt-1 flex items-center gap-1">
                          ...
                          {pack.image_count} image{pack.image_count !== 1 ? 's' : ''}
                        </div>
                        {pack.description && (
