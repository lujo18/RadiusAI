...
function DialogOverlay({ className, ...props }: DialogOverlayProps) {
  return (
    <DialogOverlayPrimitive
      className={cn('fixed inset-0 !z-[99] bg-black/50', className)}
      {...props}
    />
  );
}
  ...
  return (
    <DialogPortalPrimitive>
      ...
      <DialogContentPrimitive
        className={cn(
          'fixed top-[50%] left-[50%] !z-[100] grid w-full max-w-[calc(100%-2rem)] !translate-x-[-50%] !translate-y-[-50%] gap-4 rounded-lg border p-6 shadow-lg bg-card text-card-foreground sm:max-w-lg',
          className,
        )}
        {...props}
      >
