...
def get_recent_team_activity(team_id: str) -> Optional[Dict[str, Any]]:
    """Fetch the most recent team_activity row for a team_id."""
    ...
    try:
        ...
        print("get_team_activity failed", e)
        return None


def increment_usage_via_rpc(
    team_id: str,
    metric: str,
    amount: int = 1,
...
def set_team_activity_period(
    team_id: str, period_start: str, period_end: str
) -> Optional[Dict[str, Any]]:
    """Ensure the user's activity row has the provided period. Creates a row if none exists."""
    existing = get_team_activity(team_id)
    if not existing:
        return upsert_team_activity(
            team_id, period_start, period_end, post_inc=0, credit_inc=0
        )
...
def increment_usage_field(
    team_id: str, field: str, amount: int = 1
) -> Optional[Dict[str, Any]]:
...
def increment_credits(
    team_id: str, credit_type: str, amount: int = 1
) -> Optional[Dict[str, Any]]:
...
def increment_or_create_usage(
    ...
    try:
        rpc_res = increment_usage_via_rpc(
            team_id, metric, amount, period_start, period_end
        )
        if rpc_res:
            return rpc_res
    except Exception:
