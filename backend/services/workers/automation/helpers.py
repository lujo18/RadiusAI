...
async def get_template_by_id(
    template_id: UUID, brand_id: UUID
    ...
    try:
        ...
        logger.error(
            f"Error fetching template {template_id}: {type(e).__name__}: {e}",
            exc_info=True,
        )
        return None
    ...
    try:
        ...
        if response.data:
            if not is_active:
                ...
                logger.info(
                    f"Updated automation {automation_id} after failure (error_count={new_error_count})"
                )
            return response.data[0]

        logger.warning(
            f"No data returned when updating automation {automation_id} after failure"
        )
        return None

    except Exception as e:
        logger.error(
            f"Error updating automation {automation_id} after failure: {e}",
            exc_info=True,
        )
        return None
