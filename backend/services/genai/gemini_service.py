...
# Some imports are intentionally performed after runtime path hacks
# (google.genai proto packages can raise on import). Silence import
# position and grouping warnings for this module.
...
def generate_content_with_gemini(
    template: Template, brandSettings: BrandSettings, count: int = 1
) -> List[PostContent]:
    """
...
def _validate_gemini_response(generated: List[Dict], request: Dict) -> None:
    ...
    for post_idx, generated_post in enumerate(generated):
        for req_slide in request["slides"]:
            ...
            if not gen_slide:
                raise ValueError(
                    f"Post {post_idx + 1}: Gemini didn't generate slide {slide_num}"
                )
            ...
            for elem_id in req_slide["textElements"].keys():
                if elem_id not in gen_slide["textElements"]:
                    raise ValueError(
                        f"Post {post_idx + 1}: Gemini didn't fill element {elem_id} in slide {slide_num}"
                    )
...
def _convert_to_post_content(
    generated: Dict, template: Template, request: Dict
) -> PostContent:
    """
    ...
    for gen_slide in generated["slides"]:
        ...
        seq = next(
            s
            for s in template.style_config.slide_sequence
            if s.slide_number == slide_num
        )
        design = next(
            d for d in template.style_config.slide_designs if d.id == seq.design_id
        )
        ...
        for element in design.elements:
            if element.type == "text" and element.id in gen_slide["textElements"]:
                ...
                filled_elements.append(
                    {**element.dict(), "content": gen_slide["textElements"][element.id]}
                )
            else:
