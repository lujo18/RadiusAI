...
def update_post_status(
    ...
    if status == "posted":
        ...
        if social_post_id:
            update_data[
                "external_post_id"
            ] = social_post_id  # Store as external_post_id for analytics
...
def delete_post(post_id: str) -> bool:
    ...
    response = supabase.from_("posts").delete().eq("id", post_id).execute()
...
def create_posts_bulk(posts: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    response = supabase.from_("posts").insert(posts).execute()
