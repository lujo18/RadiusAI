...
Notes:
- During migration these helpers intentionally swallow errors and return
    `None` or `False` rather than raising. Disable the broad-except lint
    rule for this module to keep the noise actionable.
"""
...
def get_brand_analytics_summary(brand_id: str) -> Optional[dict]:
    ...
    try:
        ...
        posts_response = (
            supabase.table("posts").select("id").eq("brand_id", brand_id).execute()
        )
        ...
        total_impressions = sum(
            a.get("impressions", 0) for a in analytics_response.data
        )

        engagement_rates = [
            a.get("engagement_rate", 0)
            for a in analytics_response.data
            if a.get("engagement_rate")
        ]
        avg_engagement_rate = (
