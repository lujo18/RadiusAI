...
class SlideRenderer:
    ...
    def _load_font(self, family: str, size: int) -> ImageFont.FreeTypeFont:
        ...
        font_file = font_map.get(
            family, "TikTokSans-Medium.ttf"
        )  # Default to TikTok Sans
        ...
        print(
            f"[FontLoader] Attempting local font path: {local_font_path} exists={local_font_path.exists()}"
        )
        print(
            f"[FontLoader] Attempting docker font path: {docker_font_path} exists={docker_font_path.exists()}"
        )
        ...
        try:
            ...
            print(
                f"[FontLoader] Failed to load font '{font_path}' for family '{family}': {e}. Using default font."
            )
            return ImageFont.load_default()

    def _fetch_image_with_retry(
        self, url: str, max_retries: int = 3, timeout: int = 10
    ):
        ...
        for attempt in range(max_retries):
            try:
                ...
                print(
                    f"[Background] Timeout fetching {url} (attempt {attempt + 1}/{max_retries})"
                )
                ...
                print(
                    f"[Background] Connection error: {e} (attempt {attempt + 1}/{max_retries})"
                )
                if attempt < max_retries - 1:
        ...
        if bg_config.type == "solid" and bg_config.color:
            ...
            try:
                ...
                bg_img = bg_img.resize(
                    (new_w, new_h), PILImageModule.Resampling.LANCZOS
                )
                ...
                print(
                    f"[Background] Failed to load background image after retries: {e}"
                )
                # Fallback to dark gradient
                self._apply_background(
                    img,
                    BackgroundConfig(
                        type="gradient", gradient_colors=["#1a1a2e", "#16213e"]
                    ),
                )
    ...
    def _draw_text_with_effects(
        self, img: Image.Image, el: TextElement, font: ImageFont.FreeTypeFont
    ) -> Image.Image:
        ...
        max_width = (
            el.width if hasattr(el, "width") and el.width else (self.width - 160)
        )
        lines = self._get_wrapped_lines(el.content, font, max_width)
        line_height = font.size * (
            el.line_height if hasattr(el, "line_height") and el.line_height else 1.2
        )
        ...
        if el.shadow_color and el.shadow_blur:
            ...
            shadow_layer = shadow_layer.filter(
                ImageFilter.GaussianBlur(radius=el.shadow_blur / 2)
            )
            ...
            for line in lines:
                ...
                for dx in range(-stroke_width, stroke_width + 1):
                    for dy in range(-stroke_width, stroke_width + 1):
                        if dx != 0 or dy != 0:
                            stroke_draw.text(
                                (x + dx, y + dy), line, font=font, fill=stroke_rgb
                            )
                y += line_height
    ...
    def _draw_wrapped_text(
        self, draw: ImageDraw.Draw, el: TextElement, font: ImageFont.FreeTypeFont
    ):
    ...
    def render_slide(self, slide_design: PostSlide) -> bytes:
        ...
        for el in slide_design.elements:
            if el.type == "text":
                ...
                if (el.stroke and el.stroke_width) or (
                    el.shadow_color and el.shadow_blur
                ):
