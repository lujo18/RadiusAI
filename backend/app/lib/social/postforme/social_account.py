...
async def make_post(
    ...
    platform_ids = []
    for integration in social_integrations or []:
        ...
        if integration_id:
            platform_ids.append(str(integration_id))
    ...
    pfm_account_ids = [str(p) for p in pfm_account_ids if p]

    if not pfm_account_ids:
        raise ValueError(
            f"No connected PostForMe account found for brand {brand_id} and platforms {platforms}"
        )
    ...
    try:
        ...
        # Keep persisted status aligned with posts enum values.
        status = "posted" if mode == "publish" else mode
        ...
        raise RuntimeError(f"PostForMe API publish failed: {e}") from e
