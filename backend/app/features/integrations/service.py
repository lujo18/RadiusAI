...
async def generate_oauth_url(platform: str, brand_id: str, state: str) -> str:
    """
...
async def complete_oauth_flow(
    db: AsyncSession, user_id: str, brand_id: str, platform: str, auth_code: str
) -> PlatformIntegration:
    """
...
async def get_integration(
    db: AsyncSession, integration_id: str, user_id: str | None = None
) -> PlatformIntegration:
...
async def publish_post(
    ...
    if integration.status != IntegrationStatus.CONNECTED.value:
        raise ValidationError(
            f"Cannot publish: integration status is {integration.status}"
        )
    ...
    if (
        ...
        try:
            new_tokens = await _refresh_access_token(
                integration.platform, integration.refresh_token
            )
    ...
    try:
        result = await _publish_to_platform(
            integration.platform, integration.access_token, post_content
        )

        logger.info(
            f"Post published to {integration.platform} via integration {integration_id}"
        )
        return result
...
async def _publish_to_platform(
    platform: str, access_token: str, post_content: dict
) -> dict:
    """
