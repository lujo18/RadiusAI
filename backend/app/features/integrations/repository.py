...
class IntegrationRepository(BaseRepository[PlatformIntegration]):
    ...
    def __init__(self, supabase=None):
        super().__init__(PlatformIntegration, supabase=supabase)
    ...
    async def get_by_brand_and_platform(
        self, db: AsyncSession, brand_id: str, platform: str
    ...
    async def get_by_status(
        self, db: AsyncSession, brand_id: str, status: str, limit: int = 50
    ) -> list[PlatformIntegration]:
    ...
    async def delete_by_id(self, db: AsyncSession, integration_id: str) -> bool:
        """Delete integration by ID"""
        integration = await db.get(PlatformIntegration, integration_id)
        if not integration:
            return False
