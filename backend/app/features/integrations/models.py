...
class PlatformIntegration(Base):
    ...
    )  # Instagram ID, TikTok ID, etc.

    # Status and publishing config
    status: Mapped[str] = mapped_column(
        String(20), default=IntegrationStatus.CONNECTED.value
    )

    # Publishing settings (stored as JSON)
    publishing_config: Mapped[dict] = mapped_column(
        JSON, default={}
    )  # Schedule, hashtags, captions, etc.
