...
class AnalyticsService:
    ...
    async def get_platform_analytics(
        self, db: AsyncSession, team_id: str, platform: str, limit: int = 100
    ) -> list[PostAnalytic]:
    ...
    async def calculate_performance_score(
        self, impressions: int, engagement: int, reach: int
    ) -> float:
        """
    ...
    async def aggregate_performance(
        ...
        try:
            ...
            performance_score = await self.calculate_performance_score(
                total_impressions, total_engagement, total_reach
            )
...
# Thin async wrappers for legacy callers that import functions directly
async def get_post_analytics(db: AsyncSession, post_id: str, limit: int = 100):
    return await analytics_service.get_post_analytics(db, post_id, limit)
...
async def track_post_analytics(
    ...
    return await analytics_service.track_post_analytics(
        db, post_id, template_id, user_id, team_id, platform, metrics, variant_set_id
    )


async def get_team_analytics(db: AsyncSession, team_id: str, limit: int = 100):
    return await analytics_service.get_team_analytics(db, team_id, limit)


async def get_platform_analytics(
    db: AsyncSession, team_id: str, platform: str, limit: int = 100
):
    return await analytics_service.get_platform_analytics(db, team_id, platform, limit)


# Backwards-compat wrapper: some legacy callers import `create_analytic_tracker`
# from `app.features.analytics.service`. The actual worker lives under
# `app.core.workers.analytics`. Re-export a thin async wrapper that delegates
# to the worker. This keeps legacy imports working during migration.
try:
    from app.core.workers.analytics import create_analytic_tracker as _create_analytic_tracker
except Exception:
    # Fallback to the older services location if available
    try:
        from backend.services.workers.analytics.create_analytic_tracker import create_analytic_tracker as _create_analytic_tracker
    except Exception:
