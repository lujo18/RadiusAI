...
from app.features.billing.models import BillingPlan, Subscription, Invoice, SubscriptionStatus
...
class BillingService:
    ...
    async def create_checkout_session(
        ...
        """
        Create Stripe checkout session for plan upgrade/downgrade.
        ...
        # Get plan
        plan = await plan_repo.get(db, payload.plan_id)
        if not plan:
            raise NotFoundError("BillingPlan", payload.plan_id)
        
        # Get or create Stripe customer
        existing_subscription = await subscription_repo.get_by_user(db, user_id)
        ...
        try:
            ...
            session = stripe.checkout.Session.create(
                ...
                metadata={"user_id": user_id, "plan_id": payload.plan_id}
            )
            ...
            raise ExternalServiceError(f"Stripe checkout failed: {str(e)}")
    ...
    async def handle_checkout_completed(
        ...
        try:
            ...
            logger.info(f"Subscription created/updated for user {user_id}: {subscription.id}")
    ...
    async def get_subscription(
        ...
        return SubscriptionDetailResponse(
            ...
            can_cancel=subscription.status in [SubscriptionStatus.ACTIVE.value, SubscriptionStatus.TRIALING.value],
            can_update=subscription.status in [SubscriptionStatus.ACTIVE.value, SubscriptionStatus.TRIALING.value],
        )
        ...
        try:
            ...
            if existing:
                ...
                if stripe_invoice.paid_at:
                    existing.paid_at = datetime.fromtimestamp(stripe_invoice.paid_at, tz=UTC)
                ...
                invoice = Invoice(
                    ...
                    paid_at=datetime.fromtimestamp(stripe_invoice.paid_at, tz=UTC) if stripe_invoice.paid_at else None,
                )
