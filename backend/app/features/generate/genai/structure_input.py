...
def build_gemini_slide_structure(
    template: Template, brand_settings: BrandSettings, count: int
) -> dict[str, Any]:
    """
    ...
    for sequence in style_config.slide_sequence:
        design = next(
            (
                design
                for design in template.style_config.slide_designs
                if design.id == sequence.design_id
            ),
            None,
        )
        ...
        for element in design.elements:
            if element.type == "text":
                text_elements[element.id] = {"content": element.content}
