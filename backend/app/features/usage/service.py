...
class UsageMetricService:
    ...
    async def get_user_quota(self, db: AsyncSession, user_id: str) -> UsageQuota:
    ...
    async def check_quota(
        ...
        if limit is None:
            ...
            percentage_used = (
                min(100.0, (current_usage / limit * 100)) if limit > 0 else 0.0
            )
    ...
    async def get_usage_summary(
        self, db: AsyncSession, user_id: str, brand_id: str
    ) -> UsageSummaryResponse:
        ...
        period_active = (
            metrics.period_start is not None and metrics.period_end is not None
        )
    ...
    async def set_plan_tier(
        self, db: AsyncSession, user_id: str, plan_tier: str
    ) -> UsageQuota:
...
# Backwards-compat shim: many legacy callers import `track_slides_generated`
# from `app.features.usage.service`. Provide a thin wrapper delegating to
# the services-level implementation when available.
try:
    from services.usage.service import track_slides_generated as _track_slides_generated
except Exception:
    try:
        from backend.services.usage.service import track_slides_generated as _track_slides_generated
    except Exception:
...
def track_slides_generated(user_id: str, count: int = 1):
    if _track_slides_generated is None:
        raise RuntimeError("track_slides_generated is not available in this environment")
    return _track_slides_generated(user_id, count)


# Backwards-compat shim for check_generation_credits
try:
    from services.usage.service import check_generation_credits as _check_generation_credits
except Exception:
    try:
        from backend.services.usage.service import check_generation_credits as _check_generation_credits
    except Exception:
...
def check_generation_credits(user_id: str, slides_to_generate: int):
    if _check_generation_credits is None:
        raise RuntimeError("check_generation_credits is not available in this environment")
    return _check_generation_credits(user_id, slides_to_generate)
