...
from app.features.usage.service import UsageMetricService, get_usage_service
...
async def get_usage_summary(
    ...
    usage_service: UsageMetricService = Depends(get_usage_service),
):
    """
    ...
    """
    try:
        summary = await usage_service.get_usage_summary(user_id, brand_id)
        return summary
...
async def get_user_quota(
    ...
    """
    try:
        quota = await usage_service.get_user_quota(user_id)
...
async def consume_quota(
    ...
    try:
        result = await usage_service.consume_quota(
            user_id, brand_id, request.metric, request.amount
        )
        return result
...
async def get_brand_usage(
    ...
    """
    try:
        metric = await usage_service.get_brand_current_usage(brand_id, user_id)
        return UsageMetricResponse.model_validate(metric)
    except AppError as e:
...
async def sync_billing_period(
    ...
    """
    try:
        metric = await usage_service.sync_period_from_stripe(user_id)
        if not metric:
...
async def debug_quota_check(
    ...
    """
    try:
        check = await usage_service.check_quota(user_id, brand_id, metric, amount)
