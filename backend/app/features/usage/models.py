...
from sqlalchemy import String, Integer, Float, DateTime, JSON, ForeignKey, Index, Boolean
...
class UsageMetric(Base):
    ...
    # Metadata
    usage_metadata: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True)  # Additional tracking data
...
class UsageQuota(Base):
    ...
    user_id: Mapped[str] = mapped_column(String(255), ForeignKey("users.id"), nullable=False, unique=True)
    
    # Quota limits per metric
    slides_limit: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)  # None = unlimited
    ...
    plan_tier: Mapped[str] = mapped_column(String(50), default="free")  # free, pro, agency
