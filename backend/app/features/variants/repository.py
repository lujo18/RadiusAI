...
class VariantSetRepository(BaseRepository[VariantSet]):
    ...
    def __init__(self, supabase=None):
        super().__init__(VariantSet, supabase=supabase)

    async def get_active_by_team(self, db: AsyncSession, team_id: str) -> list[VariantSet]:
    ...
    async def get_completed_by_team(
        self, db: AsyncSession, team_id: str, limit: int = 50
    ) -> list[VariantSet]:
        """Get completed variant sets for a team."""
        stmt = (
            select(VariantSet)
            .where(and_(VariantSet.team_id == team_id, VariantSet.status == "completed"))
...
class VariantPerformanceRepository(BaseRepository[VariantPerfornance]):
    ...
    def __init__(self, supabase=None):
        super().__init__(VariantPerfornance, supabase=supabase)
...
__all__ = ["VariantSetRepository", "VariantPerformanceRepository"]
