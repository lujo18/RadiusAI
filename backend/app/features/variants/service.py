...
class VariantsService:
    ...
    async def create_variant_set(
        ...
        try:
            ...
            logger.info(f"Variant set created: {set_id} with {len(templates)} templates")
    ...
    async def complete_variant_set(
        ...
        try:
            ...
            performances = await variant_perf_repo.get_by_variant_set(db, variant_set_id)
            ...
            logger.info(f"Variant set completed: {variant_set_id}, winner: {winner.template_id}")
            return variant_set
    ...
    def _generate_insights(
        self,
        performances: list[VariantPerfornance],
        winner: VariantPerfornance
    ) -> list[str]:
        """Generate insights from variant performance data."""
        insights = []
        
        # Winner insight
        margin = ((winner.overall_score - min(p.overall_score for p in performances)) / 
                  min(p.overall_score for p in performances if p.overall_score > 0) * 100)
        insights.append(f"Winner outperforms by {margin:.1f}%")
        
        # Engagement insight
        if winner.avg_engagement_rate > 5.0:
            insights.append("High engagement template - consider replicating structure for other campaigns")
        
        # Confidence insight
        if winner.confidence_score > 0.95:
            insights.append("Strong confidence in results - template is statistically significant winner")
