...
class Template(Base):
    ...
    status: Mapped[str] = mapped_column(String(20), default="active")  # active, archived, draft
    
    # Template configuration (stored as JSON)
    style_config: Mapped[dict | None] = mapped_column(JSON, nullable=True)  # Slide designs, layouts
    content_rules: Mapped[dict | None] = mapped_column(JSON, nullable=True)  # Tone, depth, format
    
    # Template settings
    is_default: Mapped[bool] = mapped_column(Boolean, default=False)
    favorite: Mapped[bool] = mapped_column(Boolean, default=False)
    parent_id: Mapped[str | None] = mapped_column(String(50), nullable=True)  # For template inheritance
    
    # Metadata
    tags: Mapped[list[str] | None] = mapped_column(JSON, nullable=True)  # Search/organization
    performance_metrics: Mapped[dict] = mapped_column(JSON, default={})  # Cached analytics
