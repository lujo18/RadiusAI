...
class Team(Base):
    ...
    # Settings
    team_settings: Mapped[Optional[dict]] = mapped_column(
        JSON, nullable=True
    )  # Team-wide settings
    ...
    events: Mapped[List["TeamEvent"]] = relationship(
        "TeamEvent", back_populates="team", cascade="all, delete-orphan"
    )
...
class TeamMember(Base):
    ...
    team_id: Mapped[str] = mapped_column(
        String(50), ForeignKey("teams.id", ondelete="CASCADE"), nullable=False
    )
    ...
    status: Mapped[str] = mapped_column(
        String(20), default=TeamMemberStatus.ACTIVE.value
    )
    ...
    __table_args__ = (
        ...
        Index(
            "ix_team_member_team_user", "team_id", "user_id", unique=True
...
class TeamEvent(Base):
    ...
    )  # team_created, member_added, role_changed, etc
    subject_id: Mapped[Optional[str]] = mapped_column(
        String(255), nullable=True
    )  # user/member affected by event
