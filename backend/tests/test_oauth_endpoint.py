...
def test_start_endpoint():
    ...
    response = client.post(
        "/connect-social/start", json={"platform": "tiktok", "user_id": "test-user"}
    )
    ...
    assert response.status_code in [
        503,
        200,
    ], f"Expected 503 or 200, got {response.status_code}"
...
def test_validation():
    ...
    response = client.post(
        "/connect-social/start", json={"platform": "invalid_platform"}
    )
...
def test_missing_platform():
    ...
    response = client.post("/connect-social/start", json={"user_id": "test"})
